...
.sb-logo-name {
    ...
    color: #FEFCF9; letter-spacing: -0.5px;
}
.sb-logo-tagline {
    font-size: 10.5px; font-weight: 500;
    color: rgba(245,200,150,0.85); margin-top: 2px; font-style: italic;
}
...
.sb-hostel-chevron {
    flex-shrink: 0; color: rgba(254,252,249,0.35);
...
.sb-hostel-option-name {
    ...
    color: rgba(254,252,249,0.7); flex: 1;
...
.sb-hostel-dropdown-divider { height: 1px; background: rgba(254,252,249,0.06); margin: 4px 0; }
.sb-add-hostel-btn {
    display: flex; align-items: center; gap: 10px;
...
.sb-add-hostel-icon {
    ...
    font-size: 13px; flex-shrink: 0; color: #F5C896;
}
.sb-add-hostel-label { font-size: 13px; font-weight: 600; color: #F5C896; }
...
.sb-user-role { font-size: 10px; color: rgba(245,200,150,0.85); font-weight: 600; white-space: nowrap; }
...
.sb-section-label {
    ...
    color: rgba(254,252,249,0.25); position: relative; z-index: 1;
}
#hf-sidebar nav {
...
.sb-item::before {
    ...
    background: rgba(254,252,249,0.05); border-radius: 12px;
...
.sb-logout-btn:hover {
    background: rgba(168,78,32,0.22);
    border-color: rgba(168,78,32,0.4);
...
<aside id="hf-sidebar">
    ...
    @php
        ...
        $logoutRoute = $isOwnerGuard
            ? 'owner.logout'
            : ($isSuperAdmin ? 'super-admin.logout' : 'user.logout');
        ...
        /* ── MANAGER ── */
            ...
            $menu = [
                ...
                ['route' => 'manager.expenses.index',         'icon' => '💸',  'label' => 'Dépenses',         'group' => 'finance'],
        ...
        /* ── STAFF ── */
            ...
            $menu = [
                ...
                ['route' => 'staff.expenses.index',    'icon' => '💸', 'label' => 'Dépenses',     'group' => 'finance'],
            ];
            $soon = ['Rapports'];
            ...
            <div class="sb-hostel-dropdown" id="hostelDropdown">
                ...
                @foreach($allHostels as $h)
                    <form method="POST" action="{{ route('hostel.switch', $h) }}" style="margin:0">
                        @csrf
                        <button type="submit"
                                class="sb-hostel-option {{ $h->id == $activeHostelId ? 'current' : '' }}"
    ...
    {{-- ── Bouton ajout hostel (owner avec 1 seul hostel) ── --}}
    @if($isOwnerGuard && $allHostels->count() <= 1)

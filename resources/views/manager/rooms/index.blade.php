...
<div class="flex items-center justify-between mb-8 fade-up">
    <div>
        ...
        <p class="text-[15px] font-medium text-[#64748B] mt-1">Gérez les chambres de l'établissement (Manager).</p>
    </div>
    @if($currentManager->can_manage_rooms)
    <a href="{{ route('manager.rooms.create') }}" class="btn-blue fade-up delay-1">
        ...
        <tbody class="divide-y divide-[#E2E8F0]">
            ...
            <tr class="table-row-hover">
                ...
                <td class="text-right font-black text-[#0F172A]">
                    @if($room->activePrice)
                        {{ number_format($room->activePrice->price_amount, 3) }}
                        <span class="text-[#64748B] text-[12px] font-bold">{{ $room->activePrice->currency }}</span>
                    @else
                ...
                <td class="text-right space-x-3">
                    @if($currentManager->can_manage_rooms)
                    <a href="{{ route('manager.rooms.edit', $room) }}" class="inline-flex items-center justify-center p-2 rounded-xl text-[#3B82F6] hover:bg-[#EFF6FF] hover:text-[#2563EB] transition-colors" title="Modifier">
                    ...
                    <form action="{{ route('manager.rooms.destroy', $room) }}" method="POST" class="inline-block" onsubmit="return confirm('Supprimer cette chambre ?')">
                        @csrf @method('DELETE')
                        <button type="submit" class="inline-flex items-center justify-center p-2 rounded-xl text-[#EF4444] hover:bg-[#FEF2F2] hover:text-[#DC2626] transition-colors cursor-pointer bg-transparent border-none outline-none" title="Supprimer">
            ...
            <tr>
                <td colspan="7" class="py-16 text-center text-[#64748B] font-medium text-[15.5px]">
                    ...
                    Aucune chambre trouvée.<br>
                    @if($currentManager->can_manage_rooms)
                    <a href="{{ route('manager.rooms.create') }}" class="text-[#3B82F6] font-bold hover:underline mt-2 inline-block">Créer la première chambre</a>
                    @endif
                </td>
            </tr>
            @endforelse

...
<div class="flex items-center justify-between mb-8 fade-up text-[#1A2B3C]">
    <div>
        ...
        <p class="text-[15px] font-medium text-[#8A9BB0] mt-1">Gérez la tarification de base et les tarifs saisonniers (Manager).</p>
    </div>
    @if($currentManager->can_manage_pricing)
        ...
        <tbody class="divide-y divide-[#E2E8F0]">
            ...
            <tr class="table-row-hover">
                ...
                <td class="text-center">
                    <span class="inline-flex px-3 py-1 rounded-full text-[11px] font-black uppercase tracking-wider shadow-sm transition-colors duration-300 {{ $price->is_active ? 'bg-[#ECFDF5] text-[#059669] border border-[#A7F3D0]' : 'bg-[#F1F5F9] text-[#64748B] border border-[#CBD5E1]' }}">
                ...
                <td class="text-right space-x-3">
                    @if($currentManager->can_manage_pricing)
                        ...
                        @endif
                        <a href="{{ route('manager.pricing.edit', $price) }}" class="inline-flex items-center justify-center p-2 rounded-xl text-[#3B82F6] hover:bg-[#EFF6FF] transition-colors" title="Modifier">
                        ...
                        <form action="{{ route('manager.pricing.destroy', $price) }}" method="POST" class="inline-block" onsubmit="return confirm('Supprimer ce tarif ?')">
                            @csrf @method('DELETE')
                            <button type="submit" class="inline-flex items-center justify-center p-2 rounded-xl text-[#EF4444] hover:bg-[#FEF2F2] transition-colors cursor-pointer bg-transparent border-none outline-none" title="Supprimer">
...
function activatePrice(id) {
    fetch(`/manager/pricing/${id}/activate`, {
        method: 'POST',
        headers: {

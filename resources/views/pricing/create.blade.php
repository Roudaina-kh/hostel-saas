@extends('layouts.app')
@section('title', 'Ajouter un tarif (Manager)')
@section('content')

<div class="max-w-xl">
    <div class="mb-6">
        <h1 class="text-2xl font-bold" style="color: #1A2B3C;">Ajouter un tarif</h1>
        <p class="text-sm mt-1" style="color: #8A9BB0;">Configurez un nouveau tarif (Manager).</p>
    </div>
    ...
    <div class="bg-white rounded-2xl p-8 shadow-sm" style="border: 1px solid #E8EEF2;">
        ...
        {{-- Route corrigée : manager.pricing.store --}}
        <form method="POST" action="{{ route('manager.pricing.store') }}" class="space-y-5">
            @csrf
